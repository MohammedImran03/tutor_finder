...
// Check if course_id and key name are provided in the GET request
if (isset($_SESSION['courseid']) && isset($_GET['course'])) {
    // Connect to MongoDB
    ...
    $key_name = $_GET['course'];
    
    // Query to find the document based on the course_id
    $document = $collection->findOne(['_id' => new MongoDB\BSON\ObjectID($course_id)]);
    
    if ($document) {
        // Check if 'videos' exists in the document
        if (property_exists($document, 'videos')) {
            // Access the 'videos' object
            $videos = $document->videos;
            
            // Check if the key exists in the 'videos' object
            if (property_exists($videos, $key_name)) {
                // Output the value corresponding to the key as the video source
                $video_src = $videos->$key_name;
            } else {
                echo "Key not found in the 'videos' object.";
                exit;
            }
        } else {
            echo "'videos' object not found in the document.";
            exit;
        }
    } else {
    ...
    echo "Course ID or Key Name not provided.";
    exit;
}
?>
...
<html lang="en" dir="ltr">
  ...
  <body>
    <div class="container show-controls">
        ...
        <video src="Admin/template/coursevideo/<?php echo $video_src; ?>"></video>
    </div>
